...
task.normalizeMultiTaskFiles = function(data, target) {
    ...
    if (grunt.util.kindOf(data) === 'object') {
        ...
        files.push({
            src: data,
            dest: grunt.config.process(target)
        });
    }
...
function loadTasks(tasksdir) {
    try {
        var files = grunt.file.glob.sync('*.{js,coffee}', {
            cwd: tasksdir,
            maxDepth: 1
        });
...
task.loadNpmTasks = function(name) {
    ...
    var pkg = grunt.file.exists(pkgfile) ? grunt.file.readJSON(pkgfile) : {
        keywords: []
    };
...
task.init = function(tasks, options) {
    ...
    var gruntfile = allInit ? null : grunt.option('gruntfile') ||
        grunt.file.findup('Gruntfile.{js,coffee}', {
            nocase: true
        });
