...
define([
    ...
    function(EventEmitter, Device, Dom, util, Mathf, Color, Vec3, Mat4, Enums, Log, Emitter, ShaderChunks, Canvas) {
        ...
        function Renderer(opts) {
            ...
            function initEmitterBuffers(emitter) {
                ...
                if (len) {
                    if (!positionArray) {
                        positionArray = webgl.positionArray = new Float32Array(MAX * 3);
                        dataArray = webgl.dataArray = new Float32Array(MAX * 3);
                    }
            ...
            function initProgram(vertexShader, fragmentShader, parameters) {
                ...
                if (emitter) {
                    vertexHeader += "varying float vAngle;\nvarying float vAlpha;\nvarying float vSize;\n";
                    vertexMain = "\tvAngle = data.x;\n\tvAlpha= data.z;\n\tvSize = data.y;\n" + vertexMain;
                    fragmentHeader += "varying float vAngle;\nvarying float vAlpha;\nvarying float vSize;\n";
                }
            ...
            function bindEmitter(emitter, material) {
                var webgl = emitter._webgl,
                    ARRAY_BUFFER = _gl.ARRAY_BUFFER,
                    FLOAT = _gl.FLOAT,
                    attributes = material._webgl.attributes;

                if (webgl.positionBuffer && attributes.position > -1) {
                    _gl.bindBuffer(ARRAY_BUFFER, webgl.positionBuffer);
                    _gl.enableVertexAttribArray(attributes.position);
                    _gl.vertexAttribPointer(attributes.position, 3, FLOAT, false, 0, 0);
                }
                if (webgl.dataBuffer && attributes.data > -1) {
                    _gl.bindBuffer(ARRAY_BUFFER, webgl.dataBuffer);
                    _gl.enableVertexAttribArray(attributes.data);
            ...
            function renderParticleSystem(camera, lights, ambient, transform, particleSystem) {
                ...
                for (; i--;) {
                    ...
                    buffers = initEmitterBuffers(emitter);
